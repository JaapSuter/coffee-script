...
(function() {
  ...
  exports.Block = Block = (function(_super) {
    ...
    Block.prototype.compileRoot = function(o) {
      var code, exp, exports, i, prelude, preludeExps, rest;
      o.indent = o.bare ? '' : TAB;
      ...
      if (o.modularize) {
        exports = this.getModuleContext(o.modularize, o.filename);
        return "" + prelude + "\n(function(global, exports) {\n  \"use strict\";    \n  " + code + "\n}).call(undefined, window, " + exports + ");\n";
      }
      return "" + prelude + "(function() {\n" + code + "\n}).call(this);\n";
    };

    Block.prototype.getModuleContext = function(root, file) {
      var ctx, dir, ns, part, parts, recur, ttt;
      root = path.normalize(root).replace(/\\/g, '/');
      dir = path.normalize(path.dirname(file)).replace(/\\/g, '/').replace(root, '');
      file = path.basename(file, path.extname(file));
      parts = dir.split('/');
      if ('_' !== file[0]) parts.push(file);
      ns = (function() {
        var _i, _len, _results;
        _results = [];
        for (_i = 0, _len = parts.length; _i < _len; _i++) {
          part = parts[_i];
          if (part.length) _results.push(part);
        }
        return _results;
      })();
      recur = function(ns, parent) {
        var current;
        if (parent == null) parent = 'window';
        if (ns.length) {
          current = parent + '.' + ns[0];
          return "# comment to force indent underneath\n  " + current + " ?= {}              \n  " + (recur(ns.slice(1, ns.length), current));
        } else {
          return parent;
        }
      };
      ttt = recur(ns);
      ctx = "do ->\n  " + ttt;
      ctx = CoffeeScript.compile(ctx, {
        bare: true
      });
      ctx = ctx.trim();
      ctx = ctx.replace(/(\r\n|\n|\r)\s*$/gm, ' ');
      return ctx = ctx.replace(/;$/, '');
    };
  ...
  exports.Class = Class = (function(_super) {
    ...
    Class.prototype.compileNode = function(o) {
      ...
      if (decl) {
        this.body.expressions.unshift(new Assign(new Value(new Literal(name), [new Access(new Literal('__name'))]), new Literal("'" + name + "'")));
      }
